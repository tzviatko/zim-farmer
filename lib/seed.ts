...
// the database with realistic sample data across all modules.
export async function seedTestData() {
  ...
  // ── 1. Paddocks / Locations ──────────────────────────────────────────────────
  const [north, south, river, home] = await Promise.all([
  ...
  for (const animal of cattleData) {
    ...
    const ref = await addDoc(collection(db, 'cattle'), {
      ...fields,
      motherId: null,
      notes: null,
      active: true,
    ...
    if (dippedDaysAgo !== null) {
      await addDoc(collection(db, 'dip_records'), {
        animalId: ref.id,
        date: daysAgo(dippedDaysAgo),
        sessionId,
        createdAt: now(),
      })
    }
    // Weight records for the first 4
    if (['0001', '0002', '0003', '0004'].includes(fields.tag)) {
      await addDoc(collection(db, 'weight_records'), {
        animalId: ref.id,
        date: daysAgo(60),
        weightKg: 280 + Math.floor(Math.random() * 80),
        createdAt: now(),
      })
      await addDoc(collection(db, 'weight_records'), {
        animalId: ref.id,
        date: daysAgo(10),
        weightKg: 300 + Math.floor(Math.random() * 80),
        createdAt: now(),
      })
    }
  }
  ...
  for (const item of invItems) {
    ...
    await addDoc(collection(db, 'inventory_transactions'), {
      ...
      quantityIn: null,
      quantityOut: Math.round(item.parLevel * 1.2),
      createdAt: now(),
    })
  }
  ...
  const [bakkie, tractor, quad] = await Promise.all([
    addDoc(collection(db, 'vehicles'), {
      yearMakeModel: '2018 Toyota Hilux D4D',
      engine: 'Diesel',
    ...
    addDoc(collection(db, 'vehicles'), {
      yearMakeModel: '2015 John Deere 5075E',
      engine: 'Diesel',
    ...
    addDoc(collection(db, 'vehicles'), {
      yearMakeModel: '2020 Honda TRX420',
      engine: 'Petrol',
  ...
  // Mileage logs
  for (const [vehicleId, mileages] of [
    [bakkie.id,   [98200, 99100, 99850]],
    [tractor.id,  [1820, 1960, 2080]],
    [quad.id,     [12300, 12800, 13100]],
  ] as [string, number[]][]) {
    for (let i = 0; i < mileages.length; i++) {
      await addDoc(collection(db, 'mileage_logs'), {
        vehicleId,
        date: daysAgo(30 - i * 10),
        recordedMileage: mileages[i],
        notes: null,
        createdAt: now(),
      })
    }
  ...
  const [mosesRef, , blessingRef] = await Promise.all(
    staffData.map(s => addDoc(collection(db, 'staff'), {
      ...s,
      dateStarted: daysAgo(365),
      dob: null,
      address: null,
      phone: null,
  ...
  for (let i = 3; i >= 1; i--) {
    ...
    await addDoc(collection(db, 'salary_payments'), {
      ...
      amountDue: 450,
      subtractLoanRepayment: false,
      groceryBasketGiven: true,
      notes: null,
      createdAt: now(),
    })
  }
  ...
  await addDoc(collection(db, 'safety_equipment_given'), {
    staffId: mosesRef.id,
    date: daysAgo(180),
    items: ['Safety shoes', 'Work suit', 'Gloves'],
    createdAt: now(),
  })
  ...
  await Promise.all([
    addDoc(collection(db, 'revenue_entries'), {
      description: 'Sold 2 oxen to local buyer',
    ...
    addDoc(collection(db, 'revenue_entries'), {
      description: 'Maize crop sale',
      category: 'Crop sale',
      amount: 620,
      date: `${thisMonth}-03`,
      createdAt: now(),
    }),
    addDoc(collection(db, 'expense_entries'), {
      description: 'Monthly diesel purchase',
    ...
    addDoc(collection(db, 'expense_entries'), {
      description: 'Dipping chemical restock',
      category: 'Dipping chemicals',
      amount: 95,
      date: `${lastMonth}-20`,
      createdAt: now(),
    }),
  ...
  console.log('Seed complete: cattle, inventory, equipment, vehicles, staff, finance.')
}
